  ...
  class IssuedDocumentItemsList
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FattureInCloud_Ruby_Sdk::IssuedDocumentItemsList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FattureInCloud_Ruby_Sdk::IssuedDocumentItemsList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@discount.nil? && @discount > 100
        invalid_properties.push('invalid value for "discount", must be smaller than or equal to 100.')
      end
    ...
    def valid?
      return false if !@discount.nil? && @discount > 100
      return false if !@discount.nil? && @discount < 0
      true
    end
    ...
    def discount=(discount)
      if !discount.nil? && discount > 100
        fail ArgumentError, 'invalid value for "discount", must be smaller than or equal to 100.'
      end
    ...
    def hash
      [product_id, code, name, category, description, qty, measure, net_price, gross_price, vat, not_taxable, apply_withholding_taxes, discount, discount_highlight, in_ddt, stock, ei_raw].hash
    end
