...
module FattureInCloud_Ruby_Sdk
  class EntityClientPreCreateInfo < ApiModelBase
    # Countries list
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FattureInCloud_Ruby_Sdk::EntityClientPreCreateInfo` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FattureInCloud_Ruby_Sdk::EntityClientPreCreateInfo`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [countries_list, payment_methods_list, payment_accounts_list, vat_types_list, price_lists, limit, usage].hash
    end
